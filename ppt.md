...
把 AI Review 从“一次性回答”改成“受控流程”。

为什么这样设计：

1. 不是直接让模型总结整个 PR
2. 而是拆成可追踪的多个步骤
3. 模型负责分析，系统负责上下文、状态和约束
4. 每一步都能展示给用户，也能被后端校验
5. 目标不是更像聊天，而是更像真实 Review 过程
...
1. PR URL 解析  
提取 owner / repo / pullNumber

2. GitHub 数据获取  
获取 metadata、changed files 和 patch

3. Review Context 构建  
整理 PR 信息、文件 diff、统计和 warning

4. Review Pipeline 编排  
串起 context、loop、result 和状态流转

5. SSE 流式反馈  
把阶段、动作和结果实时推到前端

6. 前端工作台展示  
统一展示 PR、Diff、Timeline、风险和建议
